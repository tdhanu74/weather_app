...
const fetchWeatherData = (location: { latitude: number; longitude: number }, unit: string, setWeekWeather: Function, setCurrentWeather: Function) => {
  const params = {
  ...
  fetchWeatherApi(url, params).then((data: any) => {
    ...
    const weatherData = {
      current: {
        day: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][new Date((Number(current.time()) + utcOffsetSeconds) * 1000).getDay()],
    ...
    for (let i = 0; i < weatherData.daily.time.length; i++) {
      const date: any = weatherData.daily.time[i];
      weekWeather.push({
...
const WeatherApp = () => {
  ...
  const [weekWeather, setWeekWeather] = useState<any>(null);
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          fetch("http://ip-api.com/json").then((res) => res.json()).then((data) => {
            fetchWeatherData({latitude: data.lat, longitude: data.lon}, unit, setWeekWeather, setCurrentWeather);
          });
        }
      );
    }
            ...
            <div className="flex flex-row justify-between">
              ...
              <Tabs.Root defaultValue="C">
                <Tabs.List className="flex flex-row font-roboto sm:gap-4 md:gap-6">
                  <Tabs.Trigger
                    className="flex h-12 w-12 items-center rounded-full pl-[0.625rem] pt-[0.125rem] shadow transition duration-500 ease-in-out data-[state=active]:cursor-default data-[state=active]:bg-black-glass-2 data-[state=inactive]:bg-white-glass-2 data-[state=active]:text-white data-[state=inactive]:text-black-glass data-[state=inactive]:hover:scale-110 data-[state=active]:scale-110"
                    value="C"
                  ...
                  <Tabs.Trigger
                    className="flex h-12 w-12 items-center rounded-full pl-[0.75rem] pt-[0.125rem] shadow transition duration-500 ease-in-out data-[state=active]:cursor-default data-[state=active]:bg-black-glass-2 data-[state=inactive]:bg-white-glass-2 data-[state=active]:text-white data-[state=inactive]:text-black-glass data-[state=inactive]:hover:scale-110 data-[state=active]:scale-110"
                    value="F"
